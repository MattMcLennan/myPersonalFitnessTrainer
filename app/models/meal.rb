class Meal < ActiveRecord::Base
  ...
  def self.algo
    ...
    daily_meal.each do |meal_category, value|
      ...
      when :snack1
        ...
        self.set_meal_category_items(meal_category, "item1", find, snack1_target, SNACK1_ITEM1_ALLOCATION, item1)
        self.set_meal_category_items(meal_category, "item2", find, snack1_target, SNACK1_ITEM2_ALLOCATION, item2)
        puts "snack1"
      when :lunch
        item1 = Random.rand(find.length)
        self.set_meal_category_items(meal_category, "item1", find, lunch_target, LUNCH_ITEM1_ALLOCATION, item1)

        find = Meal.where("category LIKE ?", "Snack1")
        item2 = Random.rand(find.length)

        self.set_meal_category_items(meal_category, "item2", find, lunch_target, LUNCH_ITEM2_ALLOCATION, item2)
        puts "lunch"
      when :snack2
        item1 = Random.rand(find.length)
        item2 = Random.rand(find.length)

        self.set_meal_category_items(meal_category, "item1", find, snack2_target, SNACK2_ITEM1_ALLOCATION, item1)
        self.set_meal_category_items(meal_category, "item2", find, snack2_target, SNACK2_ITEM2_ALLOCATION, item2)
        puts "snack2"
      when :dinner
        ...
        item1 = Random.rand(find.length)
        self.set_meal_category_items(meal_category, "item1", find, dinner_target, DINNER_ITEM1_ALLOCATION, item1)
        
        find = Meal.where("category LIKE ?", "Snack2")
        item2 = Random.rand(find.length)
        self.set_meal_category_items(meal_category, "item2", find, dinner_target, DINNER_ITEM2_ALLOCATION, item2)
        puts "dinner"
      when :snack3
        item1 = Random.rand(find.length)
        self.set_meal_category_items(meal_category, "item1", find, snack3_target, SNACK3_ITEM1_ALLOCATION, item1)
  ...
  def self.set_meal_category_items(meal_category, item_num, find, meal_target, item_allocation, db_id)
    binding.pry
    meal_category[item_num][:name] = find[db_id].food
    meal_category[item_num][:grams] = ((find[db_id].per_grams / find[db_id].calories.to_f) * (meal_target * item_allocation)).round(-1)
  end
end
